...
if(isset($_COOKIE['nb_jours']) && is_numeric($_COOKIE['nb_jours'])) {
	$_SESSION['nb_jours'] = $_COOKIE['nb_jours'];
}
if (isset($_SESSION['nb_jours'])) {
	$nbJours = $_SESSION['nb_jours'];
} else {
	$nbJours = CONFIG_DEFAULT_NB_DAYS_DISPLAYED;
	$_SESSION['nb_jours'] = $nbJours;
}
...
if (isset($_SESSION['date_fin_affiche'])) {
	...
	$dateFin->modify('-' . CONFIG_DEFAULT_NB_PAST_DAYS . ' days');
...
$smarty->assign('dateDebutTexte', $smarty->get_config_vars('day_' . $dateDebut->format('w')) . ' ' . $dateDebut->format('d/m/Y'));
$smarty->assign('dateFinTexte', $smarty->get_config_vars('day_' . $dateFin->format('w')) . ' ' . $dateFin->format('d/m/Y'));
...
while ($tmpDate <= $dateFin) {
	...
	$html .= '<th style="padding-top:6px;padding-bottom:6px;" colspan="' . count($tabTranchesHoraires) . '">' . CRLF;
	$html .= $smarty->get_config_vars('planning_semaine2') . ' ' . $tmpDate->format('W') . '&nbsp;&nbsp;&nbsp; ' . $smarty->get_config_vars('planning_jour') . ' : ' . $smarty->get_config_vars('day_' . $tmpDate->format('w')) . ' ' . $tmpDate->format('d/m/Y') . CRLF;
	$html .= '</th>' . CRLF;
	$tmpDate->modify('+1 day');
}
$html .= '</tr>' . CRLF;
...
while ($tmpDate <= $dateFin) {
	...
	foreach ($tabTranchesHoraires as $trancheHeureCourante) {
		...
		$html .= '<th align="center" nowrap="nowrap" style="padding-left:2px;padding-right:2px;padding-top:6px;padding-bottom:6px;">' . $trancheHeureCourante . '-' .  $trancheFin . $smarty->get_config_vars('tab_h') . '</th>';
	}
	$tmpDate->modify('+1 day');
}
$html .= '</tr>' . CRLF;
	...
	while ($tmpDate <= $dateFin) {
		...
		foreach ($tabTranchesHoraires as $trancheHeureCourante) {
			...
			if (isset($joursOccupes[$tmpDate->format('Y-m-d')])) {
				...
				$html .= '<td ' . $styleLigne . ' valign="top" id="td_' . $ligneId . '_' . $tmpDate->format('Ymd') . '_' . $trancheHeureCourante . '"';
				if($user->checkDroit('tasks_modify_all') || $user->checkDroit('tasks_modify_own_project') || $user->checkDroit('tasks_modify_own_task')) {
				...
				foreach ($joursOccupes[$tmpDate->format('Y-m-d')] as $jour) {
					...
					$html .= '<div id="c_' . $jour['periode_id'] . '_' . $tmpDate->format('Ymd') . '_' . $trancheHeureCourante . '" class="caseProjets" onmouseover="return coolTip(\'' . xss_protect(addslashes($cooltip)) . '\')"  onmouseout="nd()" onClick="event.cancelBubble=true;" onMouseUp="Reloader.stopRefresh();modifPeriode(this, ' . $jour['periode_id'] . ');" style="color:' . $couleurTexte . ';' . $opacity . ';background-color:#' . $jour['couleur'] . ';';
