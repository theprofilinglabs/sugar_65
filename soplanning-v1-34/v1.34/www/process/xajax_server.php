...
function choixIcal() {
	...
	$lienIcal = $urlSuggeree['root'] . substr($urlSuggeree['currentDir'], 0, strlen($urlSuggeree['currentDir'])-8) . 'export_ical.php?login=' . $user->login . '&hash=' . md5($user->login . '¤¤' . $user->password . '¤¤' . CONFIG_SECURE_KEY) . "&nocache";
...
function submitFormPeriode($periode_id, $projet_id, $user_id, $date_debut, $conserver_duree, $date_fin, $nb_jours, $duree, $heure_debut, $heure_fin, $matin, $apresmidi, $repetition, $dateFinRepetition, $appliquerATous, $statut_tache, $livrable, $titre, $notes, $lien, $custom, $user_id2, $user_id3) {
	$objResponse = new xajaxResponse();
	$smarty = new MySmarty();
