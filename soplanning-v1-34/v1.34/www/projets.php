...
if (isset($_GET['order']) && in_array($_GET['order'], array('nom_createur', 'nom', 'charge', 'livraison'))) {
	$order = $_GET['order'];
...
if(isset($_GET['rechercheProjet']) && $_GET['rechercheProjet'] != ''){
	$sql = "SELECT planning_projet.*, planning_groupe.nom AS nom_groupe, planning_user.nom AS nom_createur FROM planning_projet
			...
			WHERE planning_projet.nom LIKE '%" . mysql_real_escape_string($_GET['rechercheProjet']) . "%'
			OR planning_projet.iteration LIKE '%" . mysql_real_escape_string($_GET['rechercheProjet']) . "%'
